...
void ClockMatrix::move()
{
  ...
  if (!sender.sendImage(image, 8 /** CLOCKS_X * CLOCKS_Y*/))
  {
    ...
    Serial.println("Das Image konnte nicht verschickt werden.");
    return;
  }

  // Clock Image wurde erfolgreich geschickt
  this->reachesPosition = millis() + longestMoveTime + 10;
}
...
{
  return x >= 0 && x < CLOCKS_X && y >= 0 && y < CLOCKS_Y;
}

bool ClockMatrix::isMoving()
{
  return millis() < this->reachesPosition;
}