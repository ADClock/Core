...
void JSONValue::serialize(std::back_insert_iterator<std::string> oi)
{
  switch (_type)
  {
  case TypeString:
...
JSONValue &JSONValue::operator[](std::string k)
{
  _type = TypeObject;
  for (int i = 0; i < index_token; i++)
  {
  ...
  token[index_token] = new JSONValue();
  index_token++;
  return *(token[index_token - 1]);
}

JSONValue &JSONValue::operator[](std::string k) const
{
  for (int i = 0; i < index_token; i++)
  {
#ifdef DEBUG
